    ...
    public function enroll()
{
    ...
    $studentName = session()->get('userName'); // ✅ Add this if you store user name in session
    ...
    if ($enrollmentId) {
        ...
        $notificationModel->insert([
            'user_id' => $user_id,
            'message' => 'You have been enrolled in <b>' . $course['course_name'] . '</b>.',
            'is_read' => 0,
        ...
        if (!empty($course['teacher_id'])) {
            $notificationModel->insert([
                ...
                'message' => 'A new student ' . ($studentName ?? 'has') . ' enrolled in your course <b>' . $course['course_name'] . '</b>.',
                'is_read' => 0,
                'created_at' => date('Y-m-d H:i:s')
            ]);
        }
